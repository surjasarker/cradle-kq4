...
def switch_to_environment():
    ...
    if len(named_windows) == 0:
        ...
        try:
            ...
            # Sync env_region with the real window geometry
            config.env_region = (env_window.left, env_window.top,
                                  env_window.width, env_window.height)
...
def select_window(named_windows: List[TargetWindow]) -> TargetWindow:
    ...
    try:
        ...
        # Linux: no _hWnd attribute — just use the candidate directly
        logger.warn(f'Cannot find unique env window: {config.env_name}. Using index 0.')
        env_window = candidate
