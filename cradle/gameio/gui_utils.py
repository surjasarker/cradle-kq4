...
class TargetWindow():
    ...
    def _set_sizes(self, window):
        ...
        if _isWin():
            ...
            # Linux: read attributes directly from duck-typed window object
            self.left = getattr(window, "left", 0)
            self.top = getattr(window, "top", 0)
            self.width = getattr(window, "width", 1920)
            self.height = getattr(window, "height", 1080)
            self.title = getattr(window, "title", "")
    ...
    def activate(self):
        ...
        if _isWin():
            ...
            # Linux: use xprop to raise the window
            try:
                import subprocess, re as _re, os as _os
                xenv = {**_os.environ, "DISPLAY": _os.environ.get("DISPLAY", ":0")}
                root_result = subprocess.run(
                    ["xprop", "-root", "_NET_CLIENT_LIST"],
                    capture_output=True, text=True, timeout=2, env=xenv
                )
                wid_matches = _re.findall(r'0x[0-9a-fA-F]+', root_result.stdout)
                for wid in wid_matches:
                    ...
                    for line in name_result.stdout.splitlines():
                        m = _re.search(r'"(.+)"', line)
                        if m and m.group(1) == self.title:
                            subprocess.run(
                                ["xprop", "-id", wid, "-f",
                                 "_NET_ACTIVE_WINDOW", "32a",
                                 "-set", "_NET_ACTIVE_WINDOW", wid],
...
def check_window_conditions(env_window: TargetWindow):
    # On Linux or for games that don't run at exactly 1920x1080, skip strict checks
...
def get_active_window():
    ...
    if platform.system() == "Windows":
        ...
        # Linux: get active window via xprop
        try:
            import subprocess, re as _re, os as _os
            xenv = {**_os.environ, "DISPLAY": _os.environ.get("DISPLAY", ":0")}
            ...
            if m:
                ...
                for line in name_result.stdout.splitlines():
                    nm = _re.search(r'"(.+)"', line)
                    if nm:
                        title = nm.group(1)
                        break
        except Exception:
            title = "unknown"
        win = type("LinuxWindow", (), {
            "title": title, "left": 0, "top": 0, "width": 1920, "height": 1080
        })()
        return TargetWindow(win)
...
def get_named_windows(env_name) -> List[TargetWindow]:
    ...
    if _isWin():
        ...
        # Linux: use xprop to enumerate windows and match by title
        try:
            import subprocess, re as _re, os as _os
            xenv = {**_os.environ, "DISPLAY": _os.environ.get("DISPLAY", ":0")}

            # Significant words from env_name for fuzzy matching (ignore short words)
            sig_words = [w.lower() for w in clean_name.split() if len(w) > 3]
            ...
            for wid in wid_matches:
                ...
                wn_lower = win_name.lower()
                # Match if: exact substring, scummvm in title, or enough sig words overlap
                word_matches = sum(1 for w in sig_words if w in wn_lower)
                if (
                    clean_name.lower() in wn_lower
                    or wn_lower in clean_name.lower()
                    or "scummvm" in wn_lower
                    or "king" in wn_lower and "quest" in wn_lower
                    or (sig_words and word_matches >= max(1, len(sig_words) // 2))
                ):
                    ...
                    win = type("LinuxWindow", (), {
                        "title": win_name, "left": left, "top": top,
                        "width": width, "height": height
                    })()
                    windows.append(TargetWindow(win))
            return windows
        except Exception:
            return []
