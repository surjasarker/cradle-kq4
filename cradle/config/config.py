...
class Config(metaclass=Singleton):
    ...
    DEFAULT_POST_ACTION_WAIT_TIME = 1 # Currently in use in multiple places with this value
    ...
    def _set_env_window_info(self):
        ...
        if len(named_windows) == 0:
            ...
            # When multiple windows match (e.g. outer frame + inner client area),
            # prefer the smallest-area window that still has a usable size —
            # that is the innermost content window, not the decoration frame.
            usable = [w for w in named_windows if w.width >= 100 and w.height >= 100]
            if usable:
                env_window = min(usable, key=lambda w: w.width * w.height)
            else:
