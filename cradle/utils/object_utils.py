...
class CircleDetector(metaclass=Singleton):
    def __init__(self, resolution_ratio = 0.5):
        ...
        if resolution_ratio <= .67 and hasattr(cv2, 'dnn_superres'):  # need super resolution
...
if _TORCH_AVAILABLE and _GD_AVAILABLE:
    groundingdino = GroundingDINO()
    groundingdino_detect = groundingdino.detect
else:
    groundingdino = None
    def groundingdino_detect(*args, **kwargs):
        raise RuntimeError("GroundingDINO not available: torch or groundingdino not installed")
