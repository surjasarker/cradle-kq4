...
class SkillRegistry():
    ...
    def extract_function_info(self, input_string: str = "open_map()"):
        ...
        if match:
            ...
            # Fix common model output mistakes
            if raw_arguments:
            ...
            for arg in call_node.args:
                positional_values.append(ast.literal_eval(arg))

            # If only positional args given, map them to param names via skill signature
            if positional_values and not arguments:
                skill = self.skills.get(function_name)
                param_names = []
                if skill and skill.skill_function:
                    import inspect
                    try:
                        sig = inspect.signature(skill.skill_function)
                        param_names = [
                            p for p in sig.parameters
                            if sig.parameters[p].default is inspect.Parameter.empty
                            or True  # include all params
                        ]
                    except Exception:
                        pass
                if param_names:
                    arguments = {name: val for name, val in zip(param_names, positional_values)}
                else:
                    # No signature available — use generic positional keys
                    arguments = {f"arg{i}": val for i, val in enumerate(positional_values)}

            if raw_arguments and not arguments and not positional_values:
